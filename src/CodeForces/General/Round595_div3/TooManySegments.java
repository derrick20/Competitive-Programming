...
public class TooManySegments {
    public static void main(String args[]) throws Exception {
        ...
        BIT bit = new BIT(new int[200_000 + 1]);
        Arrays.sort(arr);
...
//        System.out.println(Arrays.toString(arr));
        ...
        PriorityQueue<Interval> set = new PriorityQueue<>(new Comparator<Interval>() {
            @Override
        ...
        while (i < N) {
            while (j < N && arr[i].lo == arr[j].lo) {
                ...
                bit.add(interval.lo, 1);
                bit.add(interval.hi + 1, -1);
                set.add(interval);
                j++;
            }

            while (bit.prefixSum(arr[i].lo) > K && set.size() > 0) {
                Interval lost = set.poll();
