/*
 * @author derrick20
 * SPOJ Query on a tree V, similar to Xenia and Tree
 * Hmm my solution TLEs even though it's the exact same as other people's
 * Maybe java is too slow??
 */
import java.io.*;
import java.util.*;
...
public class QTree5 {
    public static void main(String args[]) throws Exception {
        ...
        distanceToWhite = new PriorityQueue[N];
        for (int i = 0; i < N; i++) {
            distanceToWhite[i] = new PriorityQueue<>();
            distanceToWhite[i].add(oo);
            // All things start with and always have a
            // possibility of being infinitely far from a white
            // (meaning there IS NO white in THEIR SUBTREE)
        }
        color = new boolean[N];
        // all black initially
    ...
    /*todo-----------------Below is everything relevant to the queries for this problem------------------*/
    ...
    static PriorityQueue<Integer>[] distanceToWhite;
    ...
    static int query(int node) {
        int ans = distanceToWhite[node].peek();
        ...
        while (!finished) {
            ...
            ans = Math.min(ans, distanceToWhite[par].peek() + distance(node, par));
    ...
    static void update(int node) {
        ...
        if (color[node]) {
            color[node] = false;
            distanceToWhite[node].add(0);
            improved = true;
        }
        else {
            color[node] = true;
            distanceToWhite[node].remove(0);
            improved = false;
        }
        int par = centroidParent[node];
        boolean finished = false;
        while (!finished) {
            int dist = distance(node, par);
            if (!improved) {
                // Before, we were viable as a best path. Now we aren't!
                distanceToWhite[par].remove(dist);
            }
            else {
                // We are now a NEW option
                distanceToWhite[par].add(dist);
            }
            if (par == centroidParent[par]) {
                // Once we've tried using all parents from bottom to top, curr crashes into parent
                // and we have to stop
    ...
    /*todo-----------------Below is everything relevant to centroid decomposition------------------*/
