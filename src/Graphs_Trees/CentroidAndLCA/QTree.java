...
//            return "(Node, Head, Chain, Pos)\n" + toStringHelper(0, -1,0);
        ...
        private String toStringHelper(int node, int par, int level) {
            ...
            toRet += node + ": " + "Head: " + chainHead[node] + ", Chain: " + chainIndex[node] + ", Pos: " + posInBase[node] + "\n";
    ...
    static int dfs(int node) {
        ...
        for (int adj : adjList[node]) {
            if (adj != firstParents[node]) {
                ...
                // 1 last bit of set up specific to the problem: edge labelling to one node
                int index = edgeIndex[node][adj];
                edgeEnd[index] = adj; // Since we start from the root, it will never represent an edge
                // Only things on the bottom end of edges will be considered. Just a trick for updating simpler
                // by giving the NODE the responsibility of the edge, and just leaving out the root
            }
        }
