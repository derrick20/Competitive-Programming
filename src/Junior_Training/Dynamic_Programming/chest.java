...
* Has a weird run time error, but I learned the key ideas for DP base cases was the takeaway
 */
import java.io.*;
import java.util.*;
...
public class chest {
    public static void main(String[] args) throws Exception {
        ...
        memo = new long[2][66][66]; // Reuse it between problems
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 66; j++)
                for (int k = 0; k < 66; k++)
                    memo[i][j][k] = -1;
    ...
    static long[][][] memo;
    static long solve(int above, int rem, int secure) {
        if (rem < secure || secure < 0) { // This means we won't be able to secure enough of them reach the goal, so it is
            return 0; // impossible. OR that we have secured too many, so stop (Prune this essentially)
        }
        else if (secure == 0 && rem == 0) { // we have reached the correct number of secure drawers
            return 1;
        }
        else if (memo[above][rem][secure] != -1) {
            return memo[above][rem][secure]; // use the stored value
        }
        else {
            long unlocked, locked; // Looking at the two possibilities, either lock this drawer or don't, then move on
            // to a sub-problem
            if (above == 0) { // unlocked
